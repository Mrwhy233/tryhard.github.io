...
| 文件名         | 类型     | 描述                                               |
| --------------- | -------- | -------------------------------------------------- |
| **app.py**      | 主程序   | 包含爬取逻辑、日志推送、历史记录接口等功能         |
| **history.json**| 缓存文件 | 自动生成的本地历史缓存，用于保存浏览与爬取记录     |
| **index.html**  | 前端页面 | 网页端交互界面，支持分页展示正文与链接内容         |
...
| **/history/export/index**| GET     | 下载单条记录的 JSON 文件 |
...
| **懒加载滚动** | 模拟滚动行为以触发动态内容加载                          |
| **流式通信**   | 通过 **SSE (text/event-stream)** 实现边爬边显示         |
| **内容去重**   | 使用 **dict.fromkeys()** 去除重复段落与链接             |
| **持久化存储** | 将结果保存至 JSON 文件 **history.json**                 |
...
| 📈 **统计面板**     | 展示爬取数量、来源域名分布及时间趋势                |
| 📤 **导出格式**     | 支持导出为 **CSV**、**Excel** 等格式                |
| ⚙️ **数据库版本**   | 使用 **SQLite** 存储历史数据，支持分页查询           |
| 🤖 **异步任务**     | 基于 **Celery** 实现多任务并行爬取                  |
