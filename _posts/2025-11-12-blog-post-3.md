---
title: '强国学习--day3'
date: 2025-11-12
permalink: /posts/2025/11/blog-post-3/
tags:
  - subnet
  - python
  - network
---

# 🌐 子网划分

今天复习一下子网划分，顺便写了个小工具
---

## 🧩 一、子网划分的核心概念

一个 IPv4 地址（如 `192.168.10.25`）是 32 位二进制：
```js
11000000.10101000.00001010.00011001
```

它被分为两部分：
```js
[ 网络位 | 主机位 ]
```
**掩码 (Subnet Mask)** 告诉我们网络位占多少。  
例如：  
`255.255.255.0` → `/24`，即前 24 位是网络位。  

---


## 🧮 二、计算原理步骤

假设主网段是：
```js
192.168.0.0/24 → 有 256 个地址（0–255）
```

有三个子网需求：
| 名称 | 主机需求 |
|------|-----------|
| HQ | 100 |
| BranchA | 50 |
| BranchB | 20 |

### Step 1️⃣：排序（从大到小）
先排序，确保大子网先分配，避免碎片浪费：
```js
HQ (100)，BranchA (50)，BranchB (20)
```

---

### Step 2️⃣：计算每个子网的最小掩码

> 子网能容纳主机数 = 2^(主机位数) - 2  
> （减 2 是因为要去掉网络地址和广播地址不能分配）

我们要求出 **满足“主机数 ≥ 需求数”的最小主机位数**。

#### （1）HQ 需要 100 台主机  
找最小满足条件的主机位数：

| 主机位数 | 可用主机数 | 是否够用 |
|-----------|-------------|-----------|
| 5 | 30 | ❌ |
| 6 | 62 | ❌ |
| 7 | 126 | ✅ |

→ 7 位主机位  
→ 子网掩码 = 32 - 7 = **/25**  
→ 即 `255.255.255.128`

此时：
```js
网络地址：192.168.0.0
广播地址：192.168.0.127
可用IP：192.168.0.1 – 192.168.0.126
```

---

#### （2）BranchA 需要 50 台主机
| 主机位数 | 可用主机数 | 结果 |
|-----------|-------------|------|
| 5 | 30 | ❌ |
| 6 | 62 | ✅ |

→ /26 掩码（255.255.255.192）

上一个子网到 `.127` 结束，  
下一个子网从 `.128` 开始：
```js
网络地址：192.168.0.128
广播地址：192.168.0.191
可用IP：192.168.0.129 – 192.168.0.190
```

---

#### （3）BranchB 需要 20 台主机
| 主机位数 | 可用主机数 | 结果 |
|-----------|-------------|------|
| 4 | 14 | ❌ |
| 5 | 30 | ✅ |

→ /27 掩码（255.255.255.224）

起点是 `.192`：
```js
网络地址：192.168.0.192
广播地址：192.168.0.223
可用IP：192.168.0.193 – 192.168.0.222
```

---

### Step 3️⃣：记录“已分配的子网”和“剩余地址空间”

分配完第三个 `/27` 子网：  
广播到 `.223`  
所以剩余：
```js
192.168.0.224 – 192.168.0.255 → 共 32 个地址
```

---

## ⚙️ 四、程序中原理的实现

核心计算逻辑就是这几步：

```python  
required = hosts_needed + 2          # 加上网络和广播地址  
prefix_len = 32  
while 2 ** (32 - prefix_len) < required:  
    prefix_len -= 1                  # 找到最小满足地址数量的前缀  
```
然后确定子网：
```python
subnet = ipaddress.ip_network(f"{next_net}/{prefix_len}", strict=False)
```
接着更新可用起点：
```python
next_net = subnet.broadcast_address + 1
```

## 代码
```python
import tkinter as tk
from tkinter import ttk, messagebox, filedialog
import ipaddress
import csv
import os
from prettytable import PrettyTable


def calculate_vlsm(network, subnet_requirements):
    subnet_requirements.sort(key=lambda x: x[1], reverse=True)
    allocated_subnets = []
    next_net = network.network_address

    for name, hosts_needed in subnet_requirements:
        required = hosts_needed + 2
        prefix_len = 32
        while 2 ** (32 - prefix_len) < required:
            prefix_len -= 1

        try:
            subnet = ipaddress.ip_network(f"{next_net}/{prefix_len}", strict=False)
        except ValueError:
            messagebox.showerror("错误", f"无法为 {name} 分配子网，地址空间不足。")
            break

        if subnet.network_address not in network:
            messagebox.showerror("错误", f"子网 {name} 超出主网范围。")
            break

        allocated_subnets.append((name, hosts_needed, subnet))
        next_net = subnet.broadcast_address + 1

        if next_net not in network:
            break

    remaining = None
    if next_net <= network.broadcast_address:
        remaining = ipaddress.ip_network(f"{next_net}/{network.prefixlen}", strict=False)

    return allocated_subnets, remaining


class VLSMApp:
    def __init__(self, root):
        self.root = root
        self.root.title("可变长子网划分工具 (VLSM)")
        self.root.geometry("950x650")
        self.root.resizable(False, False)

        frame_input = tk.LabelFrame(root, text="基础信息", padx=10, pady=10)
        frame_input.pack(fill="x", padx=10, pady=5)

        tk.Label(frame_input, text="主网段:").grid(row=0, column=0, sticky="w")
        self.entry_network = tk.Entry(frame_input, width=20)
        self.entry_network.grid(row=0, column=1, padx=5)
        self.entry_network.insert(0, "192.168.0.0/24")

        tk.Label(frame_input, text="子网数量:").grid(row=0, column=2, sticky="w")
        self.entry_count = tk.Entry(frame_input, width=5)
        self.entry_count.grid(row=0, column=3, padx=5)
        self.entry_count.insert(0, "3")

        tk.Button(frame_input, text="添加子网输入表", command=self.create_subnet_entries).grid(row=0, column=4, padx=10)
        tk.Button(frame_input, text="清空输入", bg="#ff6666", fg="white", command=self.reset_all).grid(row=0, column=5, padx=10)

        self.frame_subnets = tk.LabelFrame(root, text="子网需求输入", padx=10, pady=10)
        self.frame_subnets.pack(fill="x", padx=10, pady=5)
        self.subnet_entries = []

        frame_btns = tk.Frame(root)
        frame_btns.pack(fill="x", pady=5)
        tk.Button(frame_btns, text="开始计算", bg="#4CAF50", fg="white", font=("Arial", 12, "bold"), command=self.calculate).pack(side="left", padx=5)
        tk.Button(frame_btns, text="导出结果 (CSV)", bg="#2196F3", fg="white", font=("Arial", 12, "bold"), command=self.export_csv).pack(side="left", padx=5)

        frame_result = tk.LabelFrame(root, text="分配结果", padx=10, pady=10)
        frame_result.pack(fill="both", expand=True, padx=10, pady=5)

        columns = ("name", "hosts_needed", "usable", "network", "mask", "broadcast", "ip_range")
        self.tree = ttk.Treeview(frame_result, columns=columns, show="headings", height=15)
        for col, name in zip(columns, ["子网名称", "需求主机数", "可用主机数", "网络地址", "子网掩码", "广播地址", "可用 IP 范围"]):
            self.tree.heading(col, text=name)
            self.tree.column(col, anchor="center", width=130)
        self.tree.pack(fill="both", expand=True)

        self.remaining_label = tk.Label(root, text="", fg="#555", font=("Arial", 10, "italic"))
        self.remaining_label.pack(pady=3)

    def create_subnet_entries(self):
        for widget in self.frame_subnets.winfo_children():
            widget.destroy()

        try:
            count = int(self.entry_count.get())
        except ValueError:
            messagebox.showerror("错误", "请输入有效的子网数量。")
            return

        self.subnet_entries = []
        for i in range(count):
            tk.Label(self.frame_subnets, text=f"子网{i+1} 名称:").grid(row=i, column=0, sticky="w")
            name_entry = tk.Entry(self.frame_subnets, width=15)
            name_entry.grid(row=i, column=1, padx=5)
            name_entry.insert(0, f"Subnet_{i+1}")

            tk.Label(self.frame_subnets, text="主机数量:").grid(row=i, column=2, sticky="w")
            hosts_entry = tk.Entry(self.frame_subnets, width=10)
            hosts_entry.grid(row=i, column=3, padx=5)
            hosts_entry.insert(0, "50")

            self.subnet_entries.append((name_entry, hosts_entry))

    def reset_all(self):
        self.entry_network.delete(0, tk.END)
        self.entry_network.insert(0, "192.168.0.0/24")

        self.entry_count.delete(0, tk.END)
        self.entry_count.insert(0, "3")

        for widget in self.frame_subnets.winfo_children():
            widget.destroy()

        for item in self.tree.get_children():
            self.tree.delete(item)

        self.remaining_label.config(text="")
        self.subnet_entries = []

    def calculate(self):
        for item in self.tree.get_children():
            self.tree.delete(item)
        self.remaining_label.config(text="")

        try:
            network = ipaddress.ip_network(self.entry_network.get(), strict=False)
        except ValueError:
            messagebox.showerror("错误", "主网段格式错误，请输入正确的CIDR格式（例：192.168.0.0/24）")
            return

        requirements = []
        for name_entry, host_entry in self.subnet_entries:
            try:
                name = name_entry.get().strip()
                hosts = int(host_entry.get())
                if not name:
                    raise ValueError
                requirements.append((name, hosts))
            except ValueError:
                messagebox.showerror("错误", "请输入有效的子网名称和主机数。")
                return

        allocated, remaining = calculate_vlsm(network, requirements)

        for name, needed, subnet in allocated:
            hosts = list(subnet.hosts())
            usable = len(hosts)
            ip_range = f"{hosts[0]} - {hosts[-1]}" if usable > 0 else "无可用IP"
            self.tree.insert("", "end", values=(
                name, needed, usable,
                str(subnet.network_address),
                f"{subnet.netmask} (/{subnet.prefixlen})",
                str(subnet.broadcast_address),
                ip_range
            ))

        if remaining:
            self.remaining_label.config(
                text=f"剩余未分配地址空间：{remaining.network_address} - {remaining.broadcast_address} ({remaining.num_addresses} 个地址)"
            )
        else:
            self.remaining_label.config(text="所有地址空间已分配完毕。")

    def export_csv(self):
        if not self.tree.get_children():
            messagebox.showwarning("提示", "没有结果可导出，请先计算。")
            return

        file_path = filedialog.asksaveasfilename(
            defaultextension=".csv",
            filetypes=[("CSV 文件", "*.csv"), ("所有文件", "*.*")]
        )
        if not file_path:
            return

        with open(file_path, "w", newline="", encoding="utf-8-sig") as f:
            writer = csv.writer(f)
            headers = ["子网名称", "需求主机数", "可用主机数", "网络地址", "子网掩码", "广播地址", "可用 IP 范围"]
            writer.writerow(headers)
            for item in self.tree.get_children():
                writer.writerow(self.tree.item(item)["values"])

        messagebox.showinfo("成功", f"结果已导出到：{os.path.abspath(file_path)}")


if __name__ == "__main__":
    root = tk.Tk()
    app = VLSMApp(root)
    root.mainloop()
```